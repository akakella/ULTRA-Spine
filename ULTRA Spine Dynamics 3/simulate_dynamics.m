...
if (noise)
    for k = 1:links
        ...
        systemStates(k, 7) = combinedState(12*(k-1)+7) + 0.0002*randn(1); % dx
        systemStates(k, 8) = combinedState(12*(k-1)+8) + 0.0002*randn(1); % dy
        systemStates(k, 9) = combinedState(12*(k-1)+9) + 0.0002*randn(1); % dz
        systemStates(k, 10) = combinedState(12*(k-1)+10) + 0.0002*randn(1); % dT
        systemStates(k, 11) = combinedState(12*(k-1)+11) + 0.0002*randn(1); % dG
        systemStates(k, 12) = combinedState(12*(k-1)+12) + 0.0002*randn(1); % dP
    end
else
    for k = 1:links
